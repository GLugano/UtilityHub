MDH.ItemGroupOptions = {
    ...
    ["EssenceElemental"] = {
        ...
        checkItemBelongsToGroup = function(itemLink)
            ...
            return MDH:ValueInTable(essenceList, itemName);
        end
    },
    ["Stone"] = {
        label = "Stone",
        checkItemBelongsToGroup = function(itemLink)
            ...
            return MDH:ValueInTable(itemsList, itemName);
        end
    },
    ["Gem"] = {
        label = "Gem",
...
function MDH:CreateNewPresetFrame()
    ...
    MDH.NewPresetFrame = CreateFrame("Frame", MDH:ApplyPrefix("NewPresetFrame"), UIParent, "DialogBoxFrame");
    ...
    MDH:AddMovableToFrame(MDH.NewPresetFrame);
    ...
    for key, value in MDH:OrderedPairs(MDH.ItemGroupOptions) do
    ...
    MDH.NewPresetFrame.ScrollFrameParent.numTabs = MDH:TableLength(MDH.NewPresetFrame.ScrollFrameParent.Tabs);

    -- After creating all tabs
    MDH:OnClickTab(tabCustomButton);

    -- Footer
    MDH.NewPresetFrame.CloseButton = CreateFrame("Button", MDH:ApplyPrefix("NewPresetFrameCloseButton"),
    ...
    MDH.NewPresetFrame.SaveButton = CreateFrame("Button", MDH:ApplyPrefix("NewPresetFrameSaveButton"),
        MDH.NewPresetFrame, "UIPanelButtonTemplate");
...
function MDH:CreateItemGroupOption(key, label, previousRef, parent)
    local checkbox = MDH:CreateCheckbox(MDH:ApplyPrefix("Checkbox" .. key), parent, label, false, function()
    end);
...
function MDH:CreateNewPresetTab(name, index, label, frameHeight, y, previousTabButton)
    local tab = CreateFrame("Button", MDH:ApplyPrefix("NewPresetTab" .. name), MDH.NewPresetFrame.ScrollFrameParent,
...
function MDH:UpdateNewPresetItemRows(tab, tempKey)
    ...
    for i, itemLink in ipairs(rows) do
        local itemId = MDH:GetItemIDFromLink(itemLink);
    ...
    if (MDH:TableLength(temp[tempKey]) == 0) then
        tab.EmptyText:Show();
    else
        tab.EmptyText:Hide();
    end
...
function MDH:AddListToTab(tab, tempKey)
    ...
    tab:SetScript("OnMouseUp", function()
        ...
        if (itemLink and not MDH:ValueInTable(temp[tempKey], itemLink)) then
...
function MDH:UpdateWithRegister(register, registerID)
    ...
    temp.items = MDH:ShallowCopyTable(register.custom or {});
    temp.itemsExclusion = MDH:ShallowCopyTable(register.exclusion or {});
...
function MDH:CreateFormField(name, labelText, parent, y)
    ...
    local input = CreateFrame("EditBox", MDH:ApplyPrefix(name), parent, "InputBoxTemplate");
...
function MDH:SavePreset()
    ...
    if (not preset.name or #preset.name < 1) then
        ...
        MDH:ShowChatNotification("Field Name is required");
        return;
    end

    if (not preset.to or #preset.to < 1) then
        MDH.NewPresetFrame.ToInput:SetFocus();
        MDH:ShowChatNotification("Field To is required");
        return;
    end
    ...
    if (MDH:TableLength(preset.custom) == 0 and (not atLeastOneCheck)) then
        MDH:ShowChatNotification("At least one rule is required");
