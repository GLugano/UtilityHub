...
local function ShouldBeVisible()
  ...
  if (db.hideOnRightClick and state.isRightMouseDown) then
    return false;
  end
...
local function GetClassColor()
  ...
  if (classColors and classColors[className]) then
    local c = classColors[className];
    return c.r, c.g, c.b;
  end
...
local function CreateRing()
  ...
  if (swipeCooldown.SetDrawBling) then
    swipeCooldown:SetDrawBling(false);
  end

  if (swipeCooldown.SetUseCircularEdge) then
    swipeCooldown:SetUseCircularEdge(true);
  end
...
local function StartGCD()
  ...
  if (not db or not db.gcdEnabled) then
    return;
  end
...
events:SetScript("OnEvent", function(self, event, unitID)
  ...
  if (event:find("UNIT_SPELLCAST") and unitID ~= "player") then
    return;
  end
